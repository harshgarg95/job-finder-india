...
_DASH_POINTER = ("📊 **Review these in the clickable dashboard:** `python -m jobfinder dashboard` "
                 "— mark Applied / Interested / Not suitable; your calls feed the next run's ranking.")


def _stamp_dashboard_pointer() -> None:
    """Append the one-line dashboard pointer to top.md's footer. Idempotent (the
    file is regenerated by score._write_outputs on every add, then re-stamped);
    written post-render like the banner — score.py's renderer stays untouched."""
    tpath = os.path.join(RESULTS, "top.md")
    if not os.path.exists(tpath):
        return
    content = open(tpath, encoding="utf-8").read()
    if "jobfinder dashboard" in content:
        return
    open(tpath, "a", encoding="utf-8").write(f"\n---\n\n{_DASH_POINTER}\n")
...
def cmd_tracker(argv: list[str]) -> int:
    ...
    _stamp_dashboard_pointer()                          # footer: where to review + mark results
